...
export class DayComponent {
    ...
    getAverageMood(): number {
        ...
        const sum = this.day.moods.reduce(
            (acc, mood) => acc + mood.moodValue,
            0
        );
    ...
    getEmoji(): string {
        ...
        return avgMood
            ? this.emojis.mood[avgMood as MoodValue]
            : this.emojis.noMood;
    }
    ...
    OpenModal() {
        ...
        this.modalRef.content.moodDeleted.subscribe((deletedMoodId: number) => {
            this.day.moods = this.day.moods?.filter(
                mood => mood.id !== deletedMoodId
            );
        });
    }
